/**
 * *****************************************************************************
 * @file    example_velocity_control.c
 * @brief   Ví dụ điều khiển tốc độ dùng VÒNG KHÉP KÍN DÒNG ĐIỆN (True FOC Current Loop)
 *          Sử dụng kiến trúc Cascade 2-Loop: Velocity -> Current FOC.
 *
 * === CÁCH SỬ DỤNG ===
 * Tệp này chứa hàm main mẫu để chạy thử nghiệm điều khiển tốc độ 1 trục.
 * Để chạy ví dụ này:
 ...
 * - Động cơ quay ở tốc độ đặt trước (ví dụ: 30 RPM). Cứ mỗi 5 giây, động cơ
 *   tự động đảo chiều quay để thử nghiệm khả năng điều khiển đáp ứng động.
 * *****************************************************************************
 */
...
/* Tốc độ quay mục tiêu (Radian/giây)
   Công thức quy đổi RPM -> Rad/s: rad_s = RPM * (2*PI / 60)
   Ví dụ: 30 RPM = 30 * (6.28318 / 60) = 3.14159 rad/s (nửa vòng/giây) */
float target_velocity_rad_s = 3.14159f;
...
int main(void)
{
  ...
  /* Cấu hình PID vòng ngoài (Velocity) */
  ExampleConfig_t ex_cfg = {
    .vel_Kp        = 3.20f,
    .vel_Ki        = 0.0f,
    .vel_Kd        = 0.0f,
    .vel_lpf_alpha = 0.9f,
    .pos_Kp        = 0.0f, /* Không sử dụng trong điều khiển tốc độ */
    .pos_Ki        = 0.0f,
    .pos_Kd        = 0.0f
  };
  ...
  while (1)
  {
    // Cập nhật Velocity Loop (Vòng Ngoài)
    Example_RunConstantVelocity(&foc_motor, &motor_enc, target_velocity_rad_s);
    
    // Tự động đảo chiều quay sau mỗi 5 giây (5000ms) để thử nghiệm
    if (HAL_GetTick() - last_dir_change >= 5000) {
      last_dir_change = HAL_GetTick();
      target_velocity_rad_s = -target_velocity_rad_s;
      printf("[SYSTEM] Direction changed! New Target Velocity: %.2f rad/s\r\n", target_velocity_rad_s);
    }

    static uint32_t last_print = 0;
    if (HAL_GetTick() - last_print >= 200) {
      last_print = HAL_GetTick();
      printf("Target Vel: %.2f rad/s | Current Vel: %.2f rad/s | Iq_meas: %.3fA | Iq_ref: %.3fA\r\n",
             target_velocity_rad_s,
             foc_motor.velocity_mech,
             foc_motor.Iq_meas,
             foc_motor.Iq_ref);
    }
    HAL_Delay(1);
