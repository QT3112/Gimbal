...
static float _gyro_sensitivity(uint8_t gyro_fsr)
{
    switch (gyro_fsr & 0xE0U) {
        case ICM42688_GYRO_FSR_2000DPS:  return ICM42688_GYRO_SENS_2000DPS;
        case ICM42688_GYRO_FSR_1000DPS:  return ICM42688_GYRO_SENS_1000DPS;
        case ICM42688_GYRO_FSR_500DPS:   return ICM42688_GYRO_SENS_500DPS;
        case ICM42688_GYRO_FSR_250DPS:   return ICM42688_GYRO_SENS_250DPS;
        case ICM42688_GYRO_FSR_125DPS:   return ICM42688_GYRO_SENS_125DPS;
        case ICM42688_GYRO_FSR_62_5DPS:  return ICM42688_GYRO_SENS_62_5DPS;
        case ICM42688_GYRO_FSR_31_25DPS: return ICM42688_GYRO_SENS_31_25DPS;
        case ICM42688_GYRO_FSR_15_625DPS:return ICM42688_GYRO_SENS_15_625DPS;
        default:                          return ICM42688_GYRO_SENS_2000DPS;
    }
}
...
static float _accel_sensitivity(uint8_t accel_fsr)
{
    switch (accel_fsr & 0xE0U) {
        case ICM42688_ACCEL_FSR_16G: return ICM42688_ACCEL_SENS_16G;
        case ICM42688_ACCEL_FSR_8G:  return ICM42688_ACCEL_SENS_8G;
        case ICM42688_ACCEL_FSR_4G:  return ICM42688_ACCEL_SENS_4G;
        case ICM42688_ACCEL_FSR_2G:  return ICM42688_ACCEL_SENS_2G;
        default:                      return ICM42688_ACCEL_SENS_16G;
    }
}
...
ICM42688_Status_t ICM42688_WriteReg(ICM42688_Handle_t *hdev, uint8_t reg, uint8_t data)
{
    ...
    _cs_low(hdev);
    HAL_StatusTypeDef ret = HAL_SPI_Transmit(hdev->hspi, tx, 2, ICM42688_SPI_TIMEOUT_MS);
    _cs_high(hdev);
...
ICM42688_Status_t ICM42688_ReadReg(ICM42688_Handle_t *hdev, uint8_t reg, uint8_t *data)
{
    ...
    _cs_low(hdev);
    HAL_StatusTypeDef ret = HAL_SPI_Transmit(hdev->hspi, &tx, 1, ICM42688_SPI_TIMEOUT_MS);
    if (ret == HAL_OK) {
...
{
    if (bank > 4U) return ICM42688_ERROR_PARAM;
                                ...
                                const ICM42688_Config_t *config)
{
    ...
    uint8_t gyro_fsr, gyro_odr, accel_fsr, accel_odr;
    if (config != NULL) {
    ...
    if (ret != ICM42688_OK) return ret;
    if (who != ICM42688_WHO_AM_I_VAL) return ICM42688_ERROR_WHO_AM_I;
    ...
    ret = ICM42688_WriteReg(hdev, ICM42688_REG_ACCEL_CONFIG0, accel_fsr | accel_odr);
    if (ret != ICM42688_OK) return ret;

    /* --- Bước 6: Bật Gyro + Accel ở chế độ Low-Noise --- */
...
ICM42688_Status_t ICM42688_ReadSensor(ICM42688_Handle_t *hdev)
{
    ...
    hdev->gyro_x_dps = (float)hdev->raw_gyro_x  / gs;
    hdev->gyro_y_dps = (float)hdev->raw_gyro_y  / gs;
    hdev->gyro_z_dps = (float)hdev->raw_gyro_z  / gs;
    hdev->accel_x_g  = (float)hdev->raw_accel_x / as;
    hdev->accel_y_g  = (float)hdev->raw_accel_y / as;
    hdev->accel_z_g  = (float)hdev->raw_accel_z / as;
    hdev->temp_c     = (float)hdev->raw_temp / ICM42688_TEMP_SENS + ICM42688_TEMP_OFFSET;
